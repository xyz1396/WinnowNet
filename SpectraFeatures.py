...
if __name__ == "__main__":
    argv = sys.argv[1:]
    try:
        opts, args = getopt.getopt(argv, "hi:1:2:s:o:t:f:c:w:d:n:")
    except:
    ...
    if len(opts) == 0:
        ...
        print("-1\t FT1 file (MS1 spectra)\n")
        print("-2\t FT2 file (MS2 spectra)\n")
        print("-s\t FT2 file (legacy alias)\n")
    ...
    if len(ft1_file) == 0 and len(ft2_file) > 0:
        guessed_ft1 = ft2_file.replace(".FT2", ".FT1")
        if os.path.exists(guessed_ft1):
            ft1_file = guessed_ft1
    if len(ft2_file) == 0 or len(ft1_file) == 0:
        raise ValueError("Both FT1 and FT2 files are required. Use -1/-2 (or -s for FT2).")

    if len(mode) == 0:
        mode = "att"

    base_name = os.path.splitext(os.path.basename(tsv_file))[0]
    theoretical_file = os.path.join(os.path.dirname(output_file) or ".", base_name + ".theory.txt")

    D_exp_ms1 = FTtoDict(ft1_file)
    D_exp_ms2 = FTtoDict(ft2_file)
